...
var (
	...
	ErrVerifierLength = errors.New("auth/pkce: code verifier length must be between 43 and 128 characters")

	// ErrInvalidMethod is returned when an unsupported code challenge method is requested.
	ErrInvalidMethod = errors.New("auth/pkce: unsupported code challenge method (use S256)")
)
...
func GenerateVerifier(length int) (string, error) {
	...
	if _, err := rand.Read(buf); err != nil {
		return "", fmt.Errorf("auth/pkce: failed to generate random bytes: %w", err)
	}
